...
videorec() {
    if [ $1 = "--screen" ]; then
        `ffmpeg -f x11grab -video_size 1600x900 -framerate 30 -thread_queue_size 1024 -i $DISPLAY -f alsa -thread_queue_size 1024 -i default -c:v libx264 -preset superfast -c:a aac -ac 2 $HOME/Videos/$(date +%Y%m%d_%H%M%S).mp4`
        ...
        `ffmpeg -f v4l2 -framerate 30 -video_size 1920x1080 -thread_queue_size 1024 -c:v mjpeg -i /dev/video0 -f alsa -thread_queue_size 1024 -i default -preset superfast -c:a aac -channels 2 $HOME/Videos/`date +%Y%m%d_%H%M%S`.mov`
